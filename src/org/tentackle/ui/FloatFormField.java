...
// $Id: FloatFormField.java 336 2008-05-09 14:40:20Z harald $
 ...
 * @author harald
 */
public class FloatFormField extends NumberFormField {
  ...
  public FloatFormField (Document doc, int columns) {
    super (doc, columns);
    setValidChars (getValidChars() + "fFeE.,");   // add delimiters to valid chars
  ...
  public Float getFormValue ()  {
    ...
    if (str != null) {
      ...
      if (str.length() > 0) {
        try {
          return new Float(format.parse(str).floatValue());
        } 
